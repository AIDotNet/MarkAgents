...
interface ClientConnectionTrendChartProps {
  data: ClientTrendDataPoint[]
  loading?: boolean
}
...
export function ClientConnectionTrendChart({ data, loading = false }: ClientConnectionTrendChartProps) {
  ...
  const getMetricData = () => {
    switch (metric) {
      ...
      case "duration":
        return {
          dataKey: "averageConnectionDuration",
          label: "平均连接时长 (秒)",
          color: "var(--chart-3)",
          gradientId: "fillDuration"
        }
      default:
        return {
  ...
  return (
    <Card className="@container/chart">
      <CardHeader>
        <CardTitle>客户端连接趋势</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/chart:block">
            查看客户端连接的趋势变化
          </span>
          <span className="@[540px]/chart:hidden">连接趋势</span>
        </CardDescription>
        <CardAction>
          <div className="flex flex-col gap-2 @[767px]/chart:flex-row @[767px]/chart:items-center">
            ...
            <Select value={metric} onValueChange={(value: any) => setMetric(value)}>
              ...
              <SelectContent>
                <SelectItem value="connections">连接次数</SelectItem>
                <SelectItem value="successRate">成功率</SelectItem>
                <SelectItem value="duration">连接时长</SelectItem>
              </SelectContent>
            </Select>
              ...
              <Tooltip
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    const data = payload[0].payload as ClientTrendDataPoint
                    return (
                      <div className="rounded-lg border bg-background p-3 shadow-sm">
                        <div className="grid grid-cols-1 gap-2">
                          ...
                          {data.clientConnections && Object.keys(data.clientConnections).length > 0 && (
                            <div className="mt-2 space-y-1">
                              <span className="text-[0.70rem] uppercase text-muted-foreground">
                                客户端分布
                              </span>
                              {Object.entries(data.clientConnections)
                                .sort(([,a], [,b]) => b - a)
                                .slice(0, 3)
                                .map(([client, count]) => (
